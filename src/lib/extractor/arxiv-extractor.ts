 ...
 */
function extractMetadata(doc: Document): { arxivId?: string; license?: string } {
    const metadata: { arxivId?: string; license?: string } = {};

    // Extract arXiv ID from watermark
    const watermark = doc.querySelector('#watermark-tr');
    if (watermark) {
        const watermarkText = watermark.textContent || '';
        const match = watermarkText.match(/arXiv:([\d\.]+v?\d*)/);
        if (match) {
