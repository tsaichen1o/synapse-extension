    ...
    return (
        <div className="absolute top-0 right-0 h-full w-96 bg-white/95 backdrop-blur-lg shadow-2xl z-10 flex flex-col p-6 border-l border-purple-200">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent break-words pr-4">{node.title || 'Untitled'}</h2>
                <button
                    onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto pb-4 space-y-4">
                ...
                {node.summary && (
                    <div className="mb-4">
                        ...
                        <p className="text-sm text-gray-700 bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-xl border border-purple-100 leading-relaxed">{node.summary}</p>
                    </div>
                )}

                {/* Structured Data (Key-Value Pairs) */}
                {node.structuredData && Object.keys(node.structuredData).length > 0 && (
                    <div className="mb-4">
                        <h3 className="font-semibold text-gray-700 mb-2 flex items-center gap-2">
                            <svg className="w-4 h-4 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="space-y-2">
                            {Object.entries(node.structuredData).map(([key, value]) => (
                ...
                {/* Created Time */}
                {node.createdAt && (
                    <p className="text-xs text-gray-400 mt-4">Saved at: {new Date(node.createdAt).toLocaleString()}</p>
                )}
            </div>
        </div>
    );
